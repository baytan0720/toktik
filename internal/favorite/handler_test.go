...
func TestFavoriteServiceImpl_Favorite(t *testing.T) {
	t.Run("favorite failed", func(t *testing.T) {
		...
		mockVideoClient.EXPECT().GetVideo(gomock.Any(), gomock.Any()).Return(&video.GetVideoRes{VideoList: []*video.VideoInfo{}}, nil).AnyTimes()
...
func TestFavoriteServiceImpl_GetVideoFavoriteInfo(t *testing.T) {
	t.Run("get favorite info failed", func(t *testing.T) {
		...
		svc := NewFavoriteServiceImpl(svcCtx)
		_, err := svc.GetVideoFavoriteInfo(context.Background(), &favorite.GetVideoFavoriteInfoReq{
			UserId:      1,
			VideoIdList: []int64{1},
		})
		assert.Error(t, err)
	...
	t.Run("success", func(t *testing.T) {
		...
		svc := NewFavoriteServiceImpl(svcCtx)
		resp, err := svc.GetVideoFavoriteInfo(context.Background(), &favorite.GetVideoFavoriteInfoReq{
			UserId:      1,
			VideoIdList: []int64{2},
		})
		assert.NoError(t, err)
		...
		assert.False(t, resp.FavoriteInfoList[0].IsFavorite)
	})
}

func TestFavoriteServiceImpl_GetUserFavoriteInfo(t *testing.T) {
	t.Run("get favorite info failed", func(t *testing.T) {
		db := testutil.NewMockDB()
		svcCtx := &ctx.ServiceContext{
			FavoriteService: favoritesvc.NewFavoriteService(func() *gorm.DB {
		...
		svc := NewFavoriteServiceImpl(svcCtx)
		_, err := svc.GetUserFavoriteInfo(context.Background(), &favorite.GetUserFavoriteInfoReq{
			UserIdList: []int64{1},
		})
		assert.Error(t, err)
	})
	t.Run("success", func(t *testing.T) {
		mockVideoClient := newMockVideoClient(t)
		mockVideoClient.EXPECT().ListVideo(gomock.Any(), gomock.Any()).Return(&video.ListVideoRes{VideoList: []*video.VideoInfo{}}, nil).AnyTimes()
		svcCtx := &ctx.ServiceContext{
			FavoriteService: newMockFavoriteService(t),
			VideoClient:     mockVideoClient,
		}
		svc := NewFavoriteServiceImpl(svcCtx)
		resp, err := svc.GetUserFavoriteInfo(context.Background(), &favorite.GetUserFavoriteInfoReq{
			UserIdList: []int64{2},
		})
		assert.NoError(t, err)
		assert.Equal(t, 1, len(resp.FavoriteInfoList))
		assert.Equal(t, int64(0), resp.FavoriteInfoList[0].FavoriteCount)
		assert.Equal(t, int64(0), resp.FavoriteInfoList[0].TotalFavorited)
	})
}
